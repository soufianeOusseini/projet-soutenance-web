...
import {
  BUS_ACTIONS,
  COLIS_ACTIONS,
  COMPANIES_ACTIONS, COMPTES_ACTIONS, CONFIGURATIONS_ACTIONS,
  DASHBOARD_ACTIONS, DRIVERS_ACTIONS, PLANNING_ACTIONS,
  TICKETS_ACTIONS,
  TRIPS_ACTIONS
} from "../sidebar/menu-actions";

@Component({
  selector: 'app-system-sidebar',
  standalone: false,
  templateUrl: './system-sidebar.component.html',
  styleUrl: './system-sidebar.component.css'
})
export class SystemSidebarComponent  implements OnInit, OnDestroy {
  menuItems: MenuItem[] = [];
  ...
  ngOnInit(): void {
    // Récupérer les rôles de l'utilisateur depuis l'Observable
  ...
  initializeMenu(): void {
    const allMenuItems = [
      ...
      },
      {
        ...
        roles: ['ROLE_COMPANY_ADMIN', 'ROLE_ADMIN']
      },
      {
        id: 4,
  ...
  private hasRequiredRole(item: MenuItem): boolean {
    if (!item.roles || item.roles.length === 0) {
      return true; // Si pas de rôle spécifié, afficher pour tous
    }
    return this.userRoles.some(role => item.roles?.includes(role?.name!));
  }
